...
func processServer(s ServerInfo) error {
	...
	// 执行清理命令
	if err := runCommand(client, "rm -rf /opt/xiaolan-cdn/conf/*"); err != nil {
	...
	remoteDir := "/opt/xiaolan-cdn/conf"
	...
	// 执行Nginx重载
	if err := runCommand(client, "/opt/xiaolan-cdn/sbin/nginx -s reload"); err != nil {
